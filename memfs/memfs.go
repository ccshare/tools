...
// LOCKS_REQUIRED(fs.mu)
func (fs *memFS) createFile(parentID fuseops.InodeID, name string, mode os.FileMode) (fuseops.ChildInodeEntry, error) {
	// Grab the parent, which we will update shortly.
...
func (fs *memFS) CreateFile(ctx context.Context, op *fuseops.CreateFileOp) (err error) {
	if op.OpContext.Pid == 0 {
		// CreateFileOp should have a valid pid in context.
		return fuse.EINVAL
	}
	fmt.Printf("createfile pinode[%v] name[%s]\n", op.Parent, op.Name)
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) ReadFile(ctx context.Context, op *fuseops.ReadFileOp) error {
	...
	fmt.Printf("ReadFile inode[%v] offset[%v]\n", op.Inode, op.Offset)
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) WriteFile(ctx context.Context, op *fuseops.WriteFileOp) error {
	if op.OpContext.Pid == 0 {
		fmt.Printf("WriteFile opctx error, inode[%v] offset[%v] data[%s]\n", op.Inode, op.Offset, op.Data)
		//return fuse.EINVAL
	}
	fmt.Printf("WriteFile inode[%v] offset[%v] data[%s]\n", op.Inode, op.Offset, op.Data)
	fs.mu.Lock()
	defer fs.mu.Unlock()
...
func (fs *memFS) SyncFile(ctx context.Context, op *fuseops.SyncFileOp) (err error) {
	if op.OpContext.Pid == 0 {
		// FlushFileOp should have a valid pid in context.
		fmt.Printf("SyncFile opctx error, inode[%v]\n", op.Inode)
		return fuse.EINVAL
	}
	fmt.Printf("SyncFile inode[%v]\n", op.Inode)
	return
}

func (fs *memFS) FlushFile(ctx context.Context, op *fuseops.FlushFileOp) (err error) {
	if op.OpContext.Pid == 0 {
		// FlushFileOp should have a valid pid in context.
		fmt.Printf("FlushFile opctx error, inode[%v]\n", op.Inode)
		return fuse.EINVAL
	}
	fmt.Printf("FlushFile inode[%v]\n", op.Inode)
	return
}
